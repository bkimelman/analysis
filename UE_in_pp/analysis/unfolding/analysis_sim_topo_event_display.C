...
void analysis_sim(std::string runtype = "mb", int start_seg = 0, int end_seg = 200, int iter = 1, std::string bkg_cut = "dijet", bool clusters = true, bool emcal_clusters = false)  {
    ...
    TH2D* h_efrac_weight;
    TFile* f_efrac = new TFile("output_efrac_reweight_run28_dijet_bkg_cut_iter_1.root","READ");
    TH2D* hefractemp = (TH2D*)f_efrac->Get("efrac_weight");
    if (!hefractemp) {
        std::cerr << "ERROR: Could not find histogram 'efrac_weight'!" << std::endl;
        exit(1);
    }
    h_efrac_weight = (TH2D*)hefractemp->Clone("efrac_weight_clone");
    h_efrac_weight->SetDirectory(0);
    f_efrac->Close();
    ...
    */
    ...
    TH2D* h_topo_event_display[40];
    TH2D* h_topo_emcal_event_display[40];
    TH2D* h_topo_ihcal_event_display[40];
    TH2D* h_topo_ohcal_event_display[40];
    for (int i = 0; i < 40; i++) {
    ...
    //for (Long64_t entry = 0; entry < 20; ++entry) {
        ...
        *calibjet_pt = std::move(recocalibpt_new);
        ...
        if (bkg_cut != "dijet" && emfrac < 1 && emfrac > 0) {
            efrac_weight = h_efrac_weight->GetBinContent(h_efrac_weight->FindBin(calibjet_pt_dijet,emfrac));
            //if (calibjet_pt->at(ind_lead) > 21.0) std::cout << "jet pt: " << calibjet_pt->at(ind_lead) << " emfrac: " << emfrac << " efrac_weight " << efrac_weight << std::endl;
        }
        ...
        */
        ...
        if (!clusters) {
            ...
            */
            ...
            // switching et_transverse clus smear for efrac systematic 
            for (int i = 0; i < clsmult; i++) {
                ...
                if (fabs(dphi) > M_PI/3.0 && fabs(dphi) < (2.0*M_PI)/3.0 && cluster_e[i]/cosh(cluster_eta[i]) > -1.0) { 
                    ...
                    et_transverse_clus_smear += cluster_e[i]/cosh(cluster_eta[i]); 
                }
            }
        ...
        for (int i = 0; i < truthpar_n; i++) {
            ...
            if (truthpar_e[i] / cosh(truthpar_eta[i]) < 0.5) { continue; } // edited to E_T < 0.5
        ...
        // update cluster smear with efrac systematic 
        fill_response_matrix(h_truth_calib_dijet_clus_smear, h_measure_calib_dijet_clus_smear, h_respmatrix_calib_dijet_clus_smear[0], h_fake_calib_dijet_clus_smear, h_miss_calib_dijet_clus_smear, h_respmatrix_calib_dijet_clus_smear[1], uni_meas_pt, calibjet_matched_dijet, uni_truth_pt, uni_meas_et_clus_smear, uni_truth_et, weight_scale*vertex_weight*efrac_weight);
        ...
        if (entry == 26478 && reco_cut && calibjet_pt_dijet) {
