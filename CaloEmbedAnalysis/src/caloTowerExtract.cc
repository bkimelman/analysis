...
caloTowerExtract::caloTowerExtract(const std::string &name)
  : SubsysReco(name)
  , m_dettype(caloTowerExtract::HCALOUT)
  , m_detector("HCALOUT")
  , m_DETECTOR(TowerInfoContainer::HCAL)
  , m_fieldname("")
  , m_runNumber(-1)
{
  std::cout << "caloTowerExtract::caloTowerExtract(const std::string &name) Calling ctor" << std::endl;
}

//____________________________________________________________________________..
caloTowerExtract::~caloTowerExtract()
{
  std::cout << "caloTowerExtract::~caloTowerExtract() Calling dtor" << std::endl;
}

//____________________________________________________________________________..
int caloTowerExtract::InitRun(PHCompositeNode *topNode)
{
...
//____________________________________________________________________________..
int caloTowerExtract::process_event(PHCompositeNode* topNode)
{
  ...
  */
  ...
  for(int caloIndex=0; caloIndex<3; caloIndex++){
    ...
    RawTowerDefs::keytype keyEmbed = 0;
	...
	unsigned int embed_key = _embed_towers[caloIndex]->encode_key(channel);
	...
	int ieta_embed = _embed_towers[caloIndex]->getTowerEtaBin(embed_key);
	int iphi_embed = _embed_towers[caloIndex]->getTowerPhiBin(embed_key);
	...
	if(caloIndex == 0){
	  ...
	  keyEmbed = RawTowerDefs::encode_towerid(RawTowerDefs::CalorimeterId::CEMC, ieta_embed, iphi_embed);
	}else if(caloIndex == 1){
	  ...
	  keyEmbed = RawTowerDefs::encode_towerid(RawTowerDefs::CalorimeterId::HCALIN, ieta_embed, iphi_embed);
	}else if(caloIndex == 2){
	  ...
	  keyEmbed = RawTowerDefs::encode_towerid(RawTowerDefs::CalorimeterId::HCALOUT, ieta_embed, iphi_embed);
	}
	...
	TowerInfo *caloinfo_embed = _embed_towers[caloIndex]->get_tower_at_channel(channel);

	//	std::cout << "got tower infos" << std::endl;

	float data_E = caloinfo_data->get_energy();
	float sim_E = caloinfo_sim->get_energy();
	float embed_E = caloinfo_embed->get_energy();
	...
	if(caloIndex == 0){
	  ...
	  embed_phi = tower_geom->get_tower_geometry(keyEmbed)->get_phi();
	  embed_eta = tower_geom->get_tower_geometry(keyEmbed)->get_eta();
	  ...
	  embed_phi = tower_geomIH->get_tower_geometry(keyEmbed)->get_phi();
	  embed_eta = tower_geomIH->get_tower_geometry(keyEmbed)->get_eta();
	  ...
	  embed_phi = tower_geomOH->get_tower_geometry(keyEmbed)->get_phi();
	  embed_eta = tower_geomOH->get_tower_geometry(keyEmbed)->get_eta();
	}
...
int caloTowerExtract::End(PHCompositeNode *topNode)
{
...
void caloTowerExtract::CreateNodeTree(PHCompositeNode *topNode)
{
  std::cout << "creating node" << std::endl;

  Fun4AllServer *se = Fun4AllServer::instance();
  PHCompositeNode *dataTopNode = se->topNode("TOPData");
  PHCompositeNode *embedTopNode = se->topNode("TOPEmbed");
  ...
  PHNodeIterator embedIter(embedTopNode);
  ...
  PHCompositeNode *dstNodeEmbed = dynamic_cast<PHCompositeNode *>(embedIter.findFirst(
      "PHCompositeNode", "DST"));
  if (!dstNodeEmbed)
  {
    std::cerr << Name() << "::" << m_detector << "::" << __PRETTY_FUNCTION__
              << "DSTembed Node missing, doing nothing." << std::endl;
    throw std::runtime_error(
        "Failed to find DSTembed node in RawTowerCalibration::CreateNodes");
  }
  ...
  for(int i=0; i<3; i++){
	...
	throw std::runtime_error(
				 "Failed to find Calib Tower node in caloTowerExtract::CreateNodes");
      }
	...
	throw std::runtime_error(
				 "Failed to find Calib Tower Sim node in caloTowerExtract::CreateNodes");
      }


    //embed
    std::string EmbedTowerNodeName = "TOWERINFO_CALIB_" + detector;
    _embed_towers[i] = findNode::getClass<TowerInfoContainerv1>(dstNodeEmbed,
                                                           EmbedTowerNodeName);
    if (!_embed_towers[i])
      {
	std::cerr << Name() << "::" << detector << "::" << __PRETTY_FUNCTION__
		  << "Tower Calib Embed Node missing, doing nothing." << std::endl;
	throw std::runtime_error(
				 "Failed to find Calib Tower Embed node in caloTowerExtract::CreateNodes");
      }
  ...
  if (!_truth_info)
    {
      ...
      throw std::runtime_error(
			       "Failed to find Calib Tower Truth node in caloTowerExtract::CreateNodes");
    }
